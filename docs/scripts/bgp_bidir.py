#!/usr/bin/python
"""
bgp_inject_withdraw.py
Inject on DDOS_ZONE_ESCALATION
Withdraw on DDOS_ZONE_DE_ESCALATION

Building Block 1 Metadata and Configuration
Building Block 2 Input Collection
Building Block 3 Trigger Check and Decision
...
WITHDRAW_EVENT = "DDOS_ZONE_DE_ESCALATION"
...
def decide(inp):
    ev = inp.get("ddos_event")
    if ev == INJECT_EVENT:
        return "inject"
    if ev == WITHDRAW_EVENT:
        return "withdraw"
    return "skip"
...
def main():
    ...
    will_act = action != "skip"

    syslog_send(
        "stage=start run_id=%s event=%s will_act=%s action=%s dst=%s proto=%s threshold=%s alert=%s"
        % (
            safe(run_id),
            safe(inp["ddos_event"]),
            str(will_act).lower(),
    ...
    try:
        ...
        if action == "withdraw":
            if (not in_bgp) and (not in_cfg):
                note = "already_withdrawn"
            else:
                syslog_send("stage=action run_id=%s action=withdraw prefix=%s asn=%s" % (safe(run_id), PREFIX, BGP_ASN))
                clideploy(sig, ["router bgp %s" % BGP_ASN, "no network %s" % PREFIX, "exit"])
                in_bgp, in_cfg = bgp_state(sig)
    ...
    syslog_send(
        "stage=end run_id=%s ok=%s duration_ms=%s action=%s prefix=%s in_bgp=%s in_cfg=%s note=%s err=%s"
        % (
            safe(run_id),
            str(ok).lower(),
            str(duration_ms),
